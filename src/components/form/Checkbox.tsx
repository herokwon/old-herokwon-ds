...
interface CheckboxProps
  extends ContentWithId,
    Omit<
      InputProps,
      'id' | 'size' | 'label' | 'helperMessage' | 'checked' | 'defaultChecked'
    > {
...
const Checkbox = forwardRef<HTMLInputElement, CheckboxProps>(function Checkbox(
  { children, id, size = 'md', heading, description, subItems = [], ...props },
  ref,
) {
  const {
    isDisabled = false,
          ...
          className={`w-max ${
            ...
            isChecked
              ...
              : errorMessage
                ...
                : 'border-light-tertiary bg-light-tertiary group-hover:border-light-blue dark:border-dark-tertiary dark:bg-dark-secondary dark:group-hover:border-dark-blue'
          } transition-colors`}
        >
          {
            <FaCheck
...
const CheckboxGroup = ({
  ...
  const allChecked = useMemo(
    () => checkboxGroupItems.reduce((acc, item) => acc && item.isChecked, true),
    [checkboxGroupItems],
  );
    ...
    >
      {subItems.map(subItem => (
        <Checkbox
          ...
          key={subItem.id}
          isSelected={
            checkboxGroupItems.find(groupItem => groupItem.id === subItem.id)
              ?.isChecked
          }
          size={size === 'lg' ? 'md' : 'sm'}
          onChange={e =>
